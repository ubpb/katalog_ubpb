class PasswordsController < ApplicationController
  ...
  def update
    ...
    if @password_form.valid? && ils_adapter.update_user(@user.ilsuserid, password: @password_form.password) && @user.clear_password_reset_token!
      flash[:success] = "Sie haben Ihr Passwort erfolgreich zurückgesetzt. Sie können Sich jetzt mit Ihrem neuen Passwort anmelden."
...
private
  ...
  def verify_password_reset_token
    ...
    if @user.blank?
      flash[:error] = "Ihr Passwort kann nicht zurückgesetzt werden. Der verwendete Link ist ungültig. Bitte starten Sie den Rücksetzvorgang erneut."
      return redirect_to(new_session_path)
    elsif @user.password_reset_token_created_at + 24.hours <= Time.zone.now
      flash[:error] = "Ihr Passwort kann nicht zurückgesetzt werden. Der verwendete Link ist älter als 24 Stunden. Bitte starten Sie den Rücksetzvorgang erneut."
      return redirect_to(new_session_path)
    end
  end
